...
namespace Devdiscourse.Controllers.Main
{
    public class ArticleController : Controller
    {
        ...
        public async Task<ActionResult> Index(string? prefix, long id, string reg = "")
        {
            ...
            // Use Redis distributed cache to avoid repeated DB hits for the same article
            string cacheKey = $"article_{id}";
            DevNews? search = null;
            var cached = await _cache.GetStringAsync(cacheKey);
            if (!string.IsNullOrEmpty(cached))
            {
                search = JsonConvert.DeserializeObject<DevNews>(cached);
            }
            else
            {
                search = await db.DevNews.AsNoTracking()
                    .Where(dn => dn.NewsId == id && dn.AdminCheck)
                    .FirstOrDefaultAsync();
                if (search != null)
                {
                    await _cache.SetStringAsync(cacheKey,
                        JsonConvert.SerializeObject(search, new JsonSerializerSettings { ReferenceLoopHandling = ReferenceLoopHandling.Ignore }),
