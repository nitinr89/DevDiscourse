...
namespace DevDiscourse.Controllers.API
{
    ...
    public class DiscourseApiController : Controller, IDisposable
    {
        ...
        {
            var skipCount = (page - 1) * 20;
            var DiscourseIndexes = db.DiscourseIndexes.Where(a => a.LivediscourseId == id).OrderBy(o => o.CreatedOn).Select(a => new { a.Id, a.Title }).Skip(skipCount).Take(20);
        ...
        {
            var skipItem = ((page - 1) * 20);
            var discourseUpdates = (from m in db.DiscourseIndexes
        ...
        public IActionResult GetDiscourseUpdates(long id, string __amp_source_origin, int? moreItemsPageIndex)
        {
            ...
            var resultData = search.Select(b => new { Id = b.Id, Title = b.Title, ImageUrl = $"/img?imageUrl={b.ImageUrl}", Description = GetAmpHtml(b.Description), CreatedOn = b.CreatedOn.ToString("dd-MM-yyyy hh:mm:ss"), hasImage = b.ImageUrl == null ? false : true });
