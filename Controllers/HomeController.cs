...
namespace DevDiscourse.Controllers
{
    public class HomeController : Controller, IDisposable
    {
        ...
        public async Task resetpassword() {
            // Core / Identity v3
            var user = await userManager.FindByIdAsync("d6e26229-1855-4957-8e0b-eba7e87380aa")
                    ?? await userManager.FindByEmailAsync("business@visionri.com")
                    ?? await userManager.FindByNameAsync("visionri");

            if (user == null)
                throw new InvalidOperationException("User not found in this database/tenant.");

            var token = await userManager.GeneratePasswordResetTokenAsync(user);
            var result = await userManager.ResetPasswordAsync(user, token, "###devd@123###");
            if (!result.Succeeded)
        ...
        public JsonResult GetAmpNewsItems(string __amp_source_origin, string sector, string region, string tag, string label, int? moreItemsPageIndex)
        {
            ...
            var result = search.OrderByDescending(m => m.CreatedOn).Select(b => new
            {
                ...
                ImageUrl = $"/img?imageUrl={b.ImageUrl}"
            });
            int pageSize = 10;
        ...
        //    }
        //}
        [OutputCache(Duration = 120, VaryByQueryKeys = new[] { "id", "reg", "filter" })]
