    ...
    public class NewsItemsViewComponent : ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync(string sector, string region, string country, string tag, string cat, string label, int? page)
        {
            try
            {
                ...
                //.Take(65)
                //.ToList();
                // return View(resultList.GroupBy(s => s.Title).Select(a => a.FirstOrDefault()).OrderByDescending(o => o.Ranking).ToList());
                var resultList = _db.RegionNewsRankings.AsNoTracking().
                    Where(a => a.DevNews.AdminCheck == true && a.DevNews.NewsLabels == label
                    && a.Region.Title == region &&
                    a.DevNews.IsSponsored == false).OrderByDescending(a => a.DevNews.CreatedOn).Select(a => new NewsViewModel
                    {
