...
namespace DevDiscourse.Controllers.Main
{
    public class DevNewsController : BaseController, IDisposable
    {
        ...
        public async Task<ActionResult> Index(
            DateTime? bfd, DateTime? afd, int? page = 1,
            string region = "", string label = "0", string sector = "0",
            string category = "0", string country = "", string source = "",
            string text = "", string uid = "", bool editorPick = false)
        {
            // viewbags (unchanged)
            ...
            // Keep the original windows, but compute once
            var cutoff = string.IsNullOrWhiteSpace(text)
                ? DateTime.Today.AddDays(-45)
                : DateTime.Today.AddDays(-5);

            // 1) Start from DevNews only (no joins yet) + NoTracking
            var q = _db.DevNews
                .AsNoTracking()
                .Where(a => a.Type == "News" && a.CreatedOn > cutoff);

            // 2) Text search — avoid ToLower (assumes CI collation in SQL Server; if CS, use EF.Functions.Like with COLLATE)
            if (!string.IsNullOrWhiteSpace(text))
                q = q.Where(a =>
                    a.Title.Contains(text) ||
                    (a.AlternateHeadline != null && a.AlternateHeadline.Contains(text)));

            // 3) Token-style filters (CSV columns). Keep semantics but inline once.
            if (label != "0")
                q = q.Where(a =>
                    a.NewsLabels != null &&
                    (a.NewsLabels == label ||
                    a.NewsLabels.StartsWith("," + label) ||
                    a.NewsLabels.EndsWith(label + ",") ||
                    a.NewsLabels.Contains("," + label + ",")));

            if (sector != "0")
                q = q.Where(a =>
                    a.Sector != null &&
                    (a.Sector == sector ||
                    a.Sector.StartsWith("," + sector) ||
                    a.Sector.EndsWith(sector + ",") ||
                    a.Sector.Contains("," + sector + ",")));

            if (category != "0")
                q = q.Where(a =>
                    a.Category != null &&
                    (a.Category == category ||
                    a.Category.StartsWith("," + category) ||
                    a.Category.EndsWith(category + ",") ||
                    a.Category.Contains("," + category + ",")));

            // 4) Prefer equality for region/country/source (index-friendly). 
            // If you truly need partials, keep Contains(), but equality is much faster.
            if (!string.Equals(region, "Global Edition", StringComparison.Ordinal) && !string.IsNullOrWhiteSpace(region))
                q = q.Where(a => a.Region == region);

            if (!string.IsNullOrWhiteSpace(country))
                q = q.Where(a => a.Country == country);

            if (!string.IsNullOrWhiteSpace(source))
                q = q.Where(a => a.OriginalSource == source);

            if (bfd.HasValue)
                q = q.Where(a => a.CreatedOn < bfd.Value);

            if (afd.HasValue)
                q = q.Where(a => a.CreatedOn > afd.Value);

            if (!string.IsNullOrWhiteSpace(uid) && uid != "null")
                q = q.Where(a => a.Creator == uid);

            if (editorPick)
                q = q.Where(a => a.EditorPick);

            // 5) Order, page, and project MINIMAL columns first (no joins yet)
            // PagedList will run COUNT + Skip/Take – keep the row light here.
            var pageSlice = (await q
                .OrderByDescending(a => a.CreatedOn)
                .Select(a => new NewsListView
                {
                    Id            = a.Id,
                    NewsId        = a.NewsId,
                    Label         = a.NewsLabels,
                    ...
                    Title         = a.Title,
                    SubTitle      = a.SubTitle,
                    Creator       = a.Creator,   // we'll fetch name in batch
                    Region        = a.Region,
                    Country       = a.Country,
                    ImageUrl      = a.ImageUrl,
                    Source        = a.Source,
                    ...
                    // ViewCount & CreatorName will be filled after paging
                    ViewCount     = 0,
                    CreatorName   = null!
                })
                .ToListAsync()).ToPagedList(pageNumber, pageSize);
            // 6) Batch-enrich the 10 rows: metadata + creator names (no N+1)
            var ids = pageSlice.Select(x => x.Id).ToList();
            var creatorIds = pageSlice.Select(x => x.Creator).Where(s => !string.IsNullOrEmpty(s)).Distinct().ToList();

            var metaDict = await _db.DevNewsMetaDatas
                .AsNoTracking()
                .Where(m => ids.Contains(m.DevNewsId))
                .Select(m => new { m.DevNewsId, m.Views })
                .ToDictionaryAsync(m => m.DevNewsId, m => m.Views);
            ...
            {
                if (metaDict.TryGetValue(row.Id, out var views))
                    row.ViewCount = views;

                if (!string.IsNullOrEmpty(row.Creator) && usersDict.TryGetValue(row.Creator, out var name))
                    row.CreatorName = name;
                else
                    row.CreatorName = row.Creator; // fallback
            }
        ...
        public async Task<ActionResult> CreateBlog([Bind("Id,Title,SubTitle,Description,NewsLabels,SubType,Category,Author,Sector,Themes,ImageUrl,ImageCopyright,ImageCaption,Region,Country,Tags,Source,SourceUrl,EditorPick,ReferenceId,IsSponsored")] DevNews devNews, IFormFile? ImageUrl, string? ChooseImage, List<string> FileTitle, List<string> FilePath, List<string> MimeType, List<string> FileSize, List<string> FileCaption, List<string> FileThumbUrl, List<string> FileDuration)
        {
            if (ModelState.IsValid)
            {
                using (var dbContextTransaction = _db.Database.BeginTransaction())
                {
                    ...
                    catch (Exception _) { dbContextTransaction.Rollback(); }
                }
            }
            ViewBag.Sector = new SelectList(_db.DevSectors.Where(a => a.Id != 8 && a.Id != 16).OrderBy(a => a.SrNo), "Id", "Title", devNews.Sector);
            ViewBag.Category = new SelectList(_db.Categories.Where(s => s.IsActive == true).OrderByDescending(o => o.SrNo), "Id", "Title", devNews.Category);
        ...
        public async Task<ActionResult> Edit(Guid? id, string ret)
        {
            ...
            if (userWork != null && userWork.WorkStage != "Image Change")
            {
                using (var dbContextTransaction = _db.Database.BeginTransaction())
                {
                    try
                    {
                        var userWorkDetails = $"{userWork.UserName} - {userWork.WorkStage}, {userWork.ColorCode}";
                        var commandText = "UPDATE DevNews SET WorkStage = @WorkStage WHERE Id = @Id";
                        var parameters = new[]
                        {
                            new SqlParameter("@WorkStage", userWorkDetails),
                            new SqlParameter("@Id", devNews.Id)
                        };
                    ...
                    try
                    {
                        ...
                        if (FileDelete != null)
                        {
                            foreach (var item in FileDelete)
                            {
                                ...
                                if (obj != null) _db.UserNewsFiles.Remove(obj);
                            }
                            _db.SaveChanges();
                        }
                        ...
                        if (TempData["IsStandout"]?.ToString() == "False" && devNews.IsStandout == true)
                        {
                            ...
                            SendMessage(devNews.Title, description, devNews.NewsId.ToString(), devNews.ImageUrl ?? "");
                        }
        ...
        public void SendMessage(string title, string desc, string newsId, string ImageUrl)
        {
            try
            {
